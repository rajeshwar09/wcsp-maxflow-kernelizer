...
//  From main.cpp, select either one by changing single line
//
//  Mathematical basis:
//    LP relaxation of MWVC on graph G is equivalen to max-flow/min-cut on bipartite double-cover Min-cut partition classifies each vertex as:
...
namespace maxflow {
  ...
  //
  //  This is using CPU static max-flow
  ...
  class KernelizerMaxflow : public Kernelizer<CCG> {
    public:
      ...
      virtual void kernelize(typename CCG::graph_t& g, std::map<typename CCG::variable_id_t, bool>& out) {
        ...
        //
        //  The CCG uses boost::listS for its vertex container so vertex_t is a pointer-like handle (not an integer) flow_network uses integer vertex IDs (0, 1, 2, ...)
        //
        //  build a mapping:
        ...
        //
        //  Using the CPU solver. After solve(), query is_on_source_side(v) for any vertex to find which side of the min-cut it falls on
        ...
        //
        //  Iterating using the saved ccg_vertices vector (before any changes), not from vertices(g) as removing vertices from a listS graph invalidates iterators
        //
