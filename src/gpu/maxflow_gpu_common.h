...
namespace maxflow {
  ...
  __global__ void gpu_initialize_kernel(int num_nodes, int num_edges, int source, const int* capacity, int* residual_capacity, int* excess, int* height) {
  ...
  __global__ void gpu_saturate_source_kernel(int src_start, int src_end, int source, const int* edge_dst, const int* capacity, int* residual_capacity, const int* reverse_index, int* excess) {
  ...
  __global__ void gpu_bfs_step_kernel(int num_nodes, int level, const int* offset, const int* edge_dst, const int* residual_capacity, const int* reverse_index, int* height, int* flag) {
  ...
  __global__ void gpu_remove_invalid_edges_kernel(int num_nodes, int source, int sink, const int* offset, const int* edge_dst, int* residual_capacity, const int* reverse_index, int* excess, const int* height) {
    int u = blockIdx.x * blockDim.x + threadIdx.x;
  ...
  __global__ void gpu_check_active_kernel(int num_nodes, int source, int sink, const int* excess, const int* height, int* flag) {
    int u = blockIdx.x * blockDim.x + threadIdx.x;
    if (u >= num_nodes || u == source || u == sink) {
      return;
    }
