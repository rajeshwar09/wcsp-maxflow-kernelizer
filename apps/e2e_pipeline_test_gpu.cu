// Same end-to-end pipeline as e2e_pipeline_test.cpp, but the kernelize stage runs the GPU topology-driven max-flow solver (KernelizerMaxflowGPU) instead of the CPU one
// Prints the same [stage] lines so benchmark_to_excel.py parses it identically; the kernelize label is [KernelizerMaxflowGPU].
//
// Usage: ./e2e_pipeline_test_gpu <file.wcsp>
...
int main(int argc, char** argv) {
  ...
  k.kernelize(g, as);
  auto t6 = clk::now(); std::cout << "[KernelizerMaxflowGPU] " << es(t5, t6) << " s\n";
